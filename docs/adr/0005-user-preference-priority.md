...
- **opt-out (수신 거부)** — 사용자가 "이런 알림은 안 받을래요" 하고 끄는 것. 마케팅은 끌 수 있지만 결제·보안 알림은 못 끄게 한다.
- **DND / quiet hours (방해금지·조용한 시간)** — 사용자가 정한 시간대(예: 밤 10시~아침 8시)엔 알림을 보류하는 것. 단, OTP 같은 보안 알림은 새벽이라도 예외로 보낸다.
- **audit log (감사 로그)** — "왜 이 알림이 안 갔는지"를 사유와 함께 남기는 기록 장부. 나중에 사용자 문의가 오면 이 기록으로 답한다.
- **멱등 + 비중복** — 같은 알림을 한 사용자가 한 채널로 딱 한 번만 받게 하는 것. 같은 내용을 4번 받는 일이 없다.
