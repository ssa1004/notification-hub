...
채널 결정에는 *세 가지 입력* 이 동시에 작용해야 합니다.
...
- 채널 우선순위가 명시되면 (예: 마케팅은 EMAIL 만) 다른 채널은 시도 자체 없음 = vendor
  비용 절감
- (단점) 한 ChannelType 당 한 채널만 선택 — 멀티 디바이스 push (한 사용자 안드로이드 + iPad)
  는 별도 처리 필요. 현재는 가장 최근 등록 토큰 1개만 사용 가정
- (단점) DND 시간이 기간 1개로 단순화 — 실제론 평일/주말 다른 윈도우, 휴일 정책 등이
  필요할 수 있음

## 다시 검토할 시점
- 멀티 디바이스 push 가 요구되면 `Recipient#channels` 의 PUSH 채널 N개 모두 fan-out
