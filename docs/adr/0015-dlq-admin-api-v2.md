...
- **bulk (대량 처리)** — 수천 건을 하나씩이 아니라 한 번에 묶어 replay/discard 하는 것. vendor가 1시간 죽었다 살아나면 격리함에 수천 건이 쌓여 일일이 누를 수 없어서다.
- **dry-run + confirm (예행연습 + 확인)** — 진짜 실행 전에 "이만큼이 대상입니다(샘플 10건)"만 먼저 보여주고, 운영자가 confirm을 눌러야 실제로 돌리는 안전장치. 수천 건이 잘못 재발송되는 사고를 막는다.
- **partial failure (부분 실패)** — 묶음 처리 중 일부만 실패하고 나머지는 성공한 상태. 각 건을 별도로 처리해 한 건의 실패가 다른 건을 무효로 만들지 않게 한다.
- **stats / bucket aggregation (집계·시간 버킷)** — 실패 건을 '시간대별·채널별·에러 종류별'로 묶어 세어 어디서 문제가 가장 많았는지 한눈에 보는 것. bucket은 '1시간 단위' 같은 묶음 구간.
- **ISO-8601 (`PT1H` 등)** — 시각과 기간을 표기하는 국제 표준. `PT1H`는 '1시간', `PT15M`은 '15분'을 뜻한다.
- **distributed lock (분산 락)** — 여러 서버가 같은 일감을 동시에 집어 중복 처리하지 않게, 한 명만 잡게 거는 잠금. bulk 동시 실행을 1건으로 제한할 때 쓴다.
- **in-memory 저장** — 진행 상황을 DB가 아니라 서버 메모리에만 임시로 두는 것. 빠르지만 서버를 재시작하면 그 정보는 사라진다.
