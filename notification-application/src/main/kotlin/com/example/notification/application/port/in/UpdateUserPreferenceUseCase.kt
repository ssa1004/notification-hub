 ...
 */
interface UpdateUserPreferenceUseCase {

    fun update(command: UpdateCommand): UserPreference

    /**
     * @property recipientId 사용자 id
     * @property kind 변경 대상 알림 종류 (null 이면 quiet hours 만 변경)
     * @property allowed null 이 아니면 해당 kind opt-in/out 변경
     * @property preferredChannels null 이 아니면 해당 kind 우선 채널 변경
     * @property quietHours null 이면 변경 안 함, "DISABLED" 같은 sentinel 은 따로 둠
     * @property disableQuietHours true 면 DND 비활성
     * @property timezone null 이면 변경 안 함
     */
    @JvmRecord
