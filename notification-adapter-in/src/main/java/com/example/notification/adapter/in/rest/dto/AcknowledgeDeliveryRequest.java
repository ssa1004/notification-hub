 ...
 * 신뢰하지 않고 길이 / 형태를 우리 측 스키마로 한 번 더 강제한다 (OWASP API10 — Unsafe Consumption
 * of APIs). DB 컬럼 한도와 일치시켜 둬서 vendor 가 비정상 응답을 줘도 ack 가 500 으로 떨어지는
 * 것이 아니라 400 으로 빠르게 거절된다.
 */
public record AcknowledgeDeliveryRequest(
        @NotNull Boolean success,
        @Size(max = 128) String vendorMessageId,
