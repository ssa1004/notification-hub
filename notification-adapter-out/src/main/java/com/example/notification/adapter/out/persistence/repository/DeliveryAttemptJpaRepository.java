...
public interface DeliveryAttemptJpaRepository extends JpaRepository<DeliveryAttemptEntity, UUID> {
     ...
     *
     * <p>:errorPattern 은 null 일 때는 무시 — JPQL 의 {@code COALESCE(NULL, NULL) = NULL} 평가가
     * DB 별로 달라 사용하지 못함. {@code (:errorPattern IS NULL OR ...)} 패턴.
     */
    @Query(
            """
            ...
            ORDER BY d.id ASC
            """)
    List<DeliveryAttemptEntity> searchExhausted(
    ...
    /** {@link #searchExhausted} 와 동일 조건의 count. dry-run 의 estimate / total 에 사용. */
    @Query(
            """
     ...
     * {@link com.example.notification.application.port.out.DeliveryAttemptRepository} 어댑터에서
    ...
    @Query(
            ...
            WHERE d.status = :status
              ...
              AND (:toInstant IS NULL OR d.createdAt <= :toInstant)
            """)
    List<DeliveryAttemptEntity> findForStats(
            @Param("status") DeliveryStatus status,
            @Param("fromInstant") Instant fromInstant,
            @Param("toInstant") Instant toInstant);
}
