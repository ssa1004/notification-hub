...
public interface OutboxEventJpaRepository extends JpaRepository<OutboxEventEntity, Long> {
    ...
    /**
     * polling 용 PENDING row 조회. {@code FOR UPDATE SKIP LOCKED} 로 현재 트랜잭션이 잠그지 못한
     ...
     *
     * <p>네이티브 쿼리로 작성한 이유: JPA 표준은 {@code jakarta.persistence.lock.timeout = -2} 힌트
     * 매핑이 driver/벤더별로 흔들리므로, PostgreSQL 과 H2(PostgreSQL mode) 가 동일 문법을
     * 안정적으로 해석하는 native SQL 이 회귀에 더 강함.
     *
     * <p>at-least-once 자체는 ADR-0004 의 정책 (consumer 측 eventId dedup) — 본 락은 정상 경로
     * 에서 부주의한 중복 발행만 차단.
     */
    @Query(
            value =
