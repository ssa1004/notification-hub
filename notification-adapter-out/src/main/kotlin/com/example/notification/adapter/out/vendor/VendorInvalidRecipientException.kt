 ...
 *
 * application 단 DispatchDeliveryService 는 이 신호를 받으면 PUSH 채널의 경우 device
 * token 을 비활성화. [VendorPermanentException] 의 subtype 이라 retry/permanent
 * 분기는 부모 클래스로 흡수되고, deactivate 분기만 더 좁게 발동.
 */
class VendorInvalidRecipientException(message: String) :
    VendorPermanentException(message),
    InvalidRecipientFailure
