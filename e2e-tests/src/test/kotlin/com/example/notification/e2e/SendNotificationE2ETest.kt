...
internal class SendNotificationE2ETest : AbstractE2ETest() {

    @Autowired lateinit var sendUseCase: SendNotificationUseCase

    @Autowired lateinit var registerDeviceTokenUseCase: RegisterDeviceTokenUseCase

    @Autowired lateinit var recipientRepository: JpaRecipientRepository

    @Autowired lateinit var attemptJpa: DeliveryAttemptJpaRepository

    @Autowired lateinit var outboxJpa: OutboxEventJpaRepository
    ...
    fun seed() {
        recipientRepository.save(
            Recipient(
                USER,
                listOf(
                    Channel(ChannelType.PUSH, "p".repeat(160)),
                    Channel(ChannelType.EMAIL, "user@example.com"),
                    Channel(ChannelType.SMS, "+821012345678"),
                ),
                Locale.KO_KR,
        ...
        registerDeviceTokenUseCase.register(
            RegisterDeviceTokenUseCase.RegisterCommand(
                USER.value,
    ...
    @Test
    fun send_then_outbox_drained_then_attempts_succeed() {
        val idem = "idem-" + UUID.randomUUID()
        val result = sendUseCase.send(
        ...
        await().atMost(Duration.ofSeconds(20)).untilAsserted {
            val attempts = attemptJpa.findByNotificationId(result.notificationId)
            assertThat(attempts).hasSize(3)
            attempts.forEach { a: DeliveryAttemptEntity ->
        ...
        await().atMost(Duration.ofSeconds(15)).untilAsserted {
            val pending = outboxJpa.findAll().count { it.status == "PENDING" }
            assertThat(pending).isZero()
        }
    }

    @Test
    fun duplicate_idempotency_key_throws() {
        val idem = "idem-dup-" + UUID.randomUUID()
        sendUseCase.send(
            SendNotificationCommand(
                idem,
                USER.value,
    ...
    companion object {
        private val USER = RecipientId("e2e-user-" + UUID.randomUUID())
    }
}
